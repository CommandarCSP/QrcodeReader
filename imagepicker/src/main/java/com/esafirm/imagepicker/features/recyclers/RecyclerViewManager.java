...
public class RecyclerViewManager {
    ...
    public RecyclerViewManager(RecyclerView recyclerView, ImagePickerConfig config, int orientation, ManagerCallback managerCallback) {
    ...
    public void setupAdapters(ArrayList<Image> selectedImages, OnImageClickListener onImageClickListener, OnFolderClickListener onFolderClickListener) {
        ...
        folderAdapter = new FolderPickerAdapter(context, imageLoader, (bucket, title) -> {
            foldersState = recyclerView.getLayoutManager().onSaveInstanceState();
            onFolderClickListener.onFolderClick(bucket, title);
        });
    }
    ...
    public boolean handleBack() {
        if (!isFolderType && recyclerView.getAdapter() instanceof ImagePickerAdapter) {
            isFolderType = true;
            if (mManagerCallback != null) {
                mManagerCallback.setAllImage(true);
            }
            return true;
        }
        return false;
    ...
    public String getTitle() {
        ...
        Log.d("title=", ConfigUtils.getFolderTitle(context, config));
...
//        }
    ...
    public void setImageAdapter(List<Image> images) {
        ...
        layoutManager = new GridLayoutManager(context, imageColumns);
        recyclerView.setLayoutManager(layoutManager);
        recyclerView.setHasFixedSize(true);
    ...
    public interface ManagerCallback {
        ...
        void setAllImage(boolean backToFolder);
